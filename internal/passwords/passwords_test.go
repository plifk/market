...
func TestValidate(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args: args{
				...
				blocked:  []string{"light-sun-window-led-bulb-galaxy"},
			},
			wantErr: true,
		},
		{
			name: "partial blocked password",
			args: args{password: "light-sun-window-x",
				blocked: []string{"light-sun-window"},
			},
			wantErr: true,
		},
		{
			name: "password has parts in blocked but should pass as it's complicated enough",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := Validate(tt.args.password, tt.args.blocked...); (err != nil) != tt.wantErr {
